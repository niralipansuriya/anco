...
class BillHistoryUseCase @Inject constructor(private val paymentRepository: PaymentRepository) :
    BaseUseCase<BaseResponse<BillHistoryResponse>>() {
    override fun buildSingle(data: Map<String, Any?>?): Single<BaseResponse<BillHistoryResponse>> {
            return paymentRepository.getBillHistory( page = data?.get(UseCaseConstants.PER_PAGE) as String)
    }
}