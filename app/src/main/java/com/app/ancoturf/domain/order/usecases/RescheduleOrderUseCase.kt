...
class RescheduleOrderUseCase @Inject constructor(private val orderRepository: OrderRepository) :
    ...
    override fun buildSingle(data: Map<String, Any?>?): Single<BaseResponse<OrderDetailResponse>> {
        return orderRepository.rescheduleOrder(orderId = data?.get(UseCaseConstants.ORDER_ID) as Int, newDeliveryDate = data?.get(UseCaseConstants.DELIVERY_DATE) as String)
    }
